...
The main configuration file is `config/train_gmamba.yaml`. Common options
include:
...
| `num_epoch` | `120` | Number of training epochs |
| `base_lr` | `0.00075` | Initial learning rate |
| `optimizer` | `AdamW` | Optimizer |
| `device` | `[0]` | GPU IDs |
| `num_worker` | `8` | DataLoader worker count |
| `window_size` | `48` | Input sequence length |
| `num_point` | `16` | Number of joints |
| `aff_dim` | `1488` | Affective feature dimension |
| `use_mamba_ssm` | `true` | Use the external Mamba SSM implementation |
| `use_layer_contrastive` | `true` | Enable layer-wise contrastive learning |
| `export_confusion` | `true` | Export confusion matrix data |
| `export_visuals` | `false` | Export visualization images |
| `export_analysis_data` | `false` | Export intermediate analysis data |

Command-line arguments override values loaded from YAML:

```bash
python main.py --config config/train_gmamba.yaml --num-epoch 60 --batch-size 8 --device 0 --save_model true
```

For multiple GPUs:

```bash
python main.py --config config/train_gmamba.yaml --device 0 1 --batch-size 32
```

For CPU debugging, use a small worker count and limit the number of batches:

```bash
python main.py --config config/train_gmamba.yaml --device 0 --num-worker 0 --max-train-batches 1 --max-eval-batches 1
```

If no CUDA device is available, the code falls back to CPU automatically.

## Model Overview

The high-level computation flow is:

```text
Pose sequence
    |
    +--> Adaptive GCN --> MTM --> Bidirectional Mamba --> Pose representation

Motion sequence
    |
    +--> Adaptive GCN --> MTM --> Bidirectional Mamba --> Motion representation

Affective feature
    |
    +--> Affective conditioning

Pose representation + Motion representation
    |
    +--> Cross-stream fusion and spatio-temporal attention pooling
    |
    +--> Classification head and affective prediction head
```

Important components in `model/gmamba.py`:

- `AdaptiveGCN`: learns an adaptive graph over the 16-joint topology.
- `MTM`: models local temporal dynamics with multi-scale temporal shifts.
- `BiMambaBlock`: performs forward and backward sequence modeling.
- `CrossStreamsGate`: exchanges information between pose and motion streams.
- `EmoConditioning`: uses affective features to condition learned
  representations.
- `DynamicMultiTaskHead`: produces class logits and affective feature
  predictions.
- `PRC`: optional layer-wise contrastive learning module.

When `mamba-ssm` is installed and `use_mamba_ssm` is enabled, the model uses
the external Mamba backend. Otherwise, it reports and uses `gmamba_mamba_lite`.
